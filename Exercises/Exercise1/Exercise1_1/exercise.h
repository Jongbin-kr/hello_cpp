...
// Your code here:
template <typename T>
vector<T> addVectors(vector<T>& v1, vector<T>& v2){  // reference를 썼다! 
    if (v1.size() != v2.size()){
        //// ERROR vector<T>를 반환한다고 헤더에 써놨기에 에러가 생긴다.
    ...
    vector<T> v3(v1.size());
    for (int i=0; i < v1.size(); i++){ // !! int가 아니라 size_t로 받아야하는군.
        v3[i] = v1[i] + v2[i];
    }
...
void processVectors() {
    ...
    ===================================================================================================*/
    // your code here:
    T dotResult = dotProduct(v1, v2);
    cout << "Result of dotProduct: " << dotResult << endl;
}
